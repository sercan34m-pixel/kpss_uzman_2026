...
def main(page: ft.Page):
    # --- KESİN ÇÖZÜM: RESMİN TAM YOLUNU BULMA ---
    # Bu kod, main.py dosyasının olduğu yeri bulur ve assets/icon.png yolunu tam olarak hesaplar.
    script_dir = os.path.dirname(os.path.abspath(__file__))
    icon_yolu = os.path.join(script_dir, "assets", "icon.png")
    
    print(f"Resim Yolu: {icon_yolu}") # Terminale yolun doğru olup olmadığını yazar
    page.window_icon = icon_yolu
    # ---------------------------------------------
    ...
    def yukle():
        if os.path.exists("flet_data.json"):
            try:
                with open("flet_data.json", "r", encoding="utf-8") as f:
    ...
    def kaydet():
        ...
        with open("flet_data.json", "w", encoding="utf-8") as f:
...
# --- 2. ANA SAYFA (GÜNCELLENDİ: TAM ORTALAMA VE HİZALAMA) ---
    ...
    def view_profil():
        ...
        def sifirla_onay(e):
            state.update({"dogru":0, "cozulen":0, "kayitlar":[], "stats":{k: {"d":0, "y":0} for k in MUFREDAT}, "cozulen_idleri": []})
            kaydet()
